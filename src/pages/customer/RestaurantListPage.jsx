    ...
    <div className="page-container">
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: 24 }}>
        <div style={{ position: "relative", flex: 1, minWidth: 200, maxWidth: 400 }}>
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search restaurants..."
            style={{ width: "100%", paddingLeft: 42, paddingRight: 16, paddingTop: 11, paddingBottom: 11, borderRadius: 12, background: "var(--bg-card)", border: "1px solid var(--border-2)", color: "var(--text-primary)", fontSize: 13, outline: "none" }} />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid-cards">
          {restaurants.map((r) => (
            <div key={r._id} onClick={() => navigate(`/customer/restaurant/${r._id}`)} className="card"
              style={{ padding: 0, overflow: "hidden", cursor: "pointer" }}>
              {/* Banner */}
              <div style={{ height: 100, background: "var(--bg-hover)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 36 }}>
                🍽️
              </div>
              <div style={{ padding: "16px 18px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  ...
                  <span style={{ padding: "2px 8px", borderRadius: 9999, fontSize: 11, fontWeight: 700, background: "#10b98122", color: "#34d399" }}>
                    OPEN
                  </span>
                </div>
